...
class raidTimeToy {
    ...
    private validateConfig(): { isValid: boolean; warnings: string[]; fixes: string[]; safeConfig: any } {
        const warnings: string[] = [];
        const fixes: string[] = [];
        let isValid = true;

        // Create a working copy of the config that can safely be modified
        const safeConfig = {
            enabled: (config as any).enabled !== false,
            raidTimeMultiplier: config.raidTimeMultiplier,
            globalMultiplier: (config as any).globalMultiplier,
            randomMode: { ...(config as any).randomMode },
            categories: { ...(config as any).categories },
            perMapSettings: { ...(config as any).perMapSettings }
        };
        ...
        // 5. Check if mod is even enabled
        if (!safeConfig.enabled) {
            warnings.push(`Mod is disabled in config`);
            fixes.push(`Set "enabled": true to activate raidTimeToy`);
            isValid = false; // Set the flag here!
        }
    ...
    public postDBLoad(container: any): void {
        ...
        if (safeConfig.randomMode?.enabled) {
            ...
            titleText = `🎮 raidTimeToy v1.0 - Global Mode (${safeConfig.raidTimeMultiplier}x)`;
            summaryText = `✅ Applied ${safeConfig.raidTimeMultiplier}x to ${modifiedCount} maps!`;
        } else {
